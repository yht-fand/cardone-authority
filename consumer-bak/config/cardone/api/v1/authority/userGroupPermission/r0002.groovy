...
class r0002 implements java.io.Serializable {
    ...
    def func(input) {
		ApplicationContextHolder.getBean(UserGroupPermissionService.class).findListByKeyword(input)
    }
    ...
    def output(output) {
        ...
        for (def outputItem : output) {
			newOutput.add(['label': outputItem['name'], 'value': outputItem['c1_user_group_permission_code']])
        }
