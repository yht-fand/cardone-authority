...
class r0003 implements java.io.Serializable {
    ...
    def func(input) {
		ApplicationContextHolder.getBean(PermissionMapperService.class).page(input)
    }
    ...
    def output(output) {
        ...
        for (def contentItem : output.getContent()) {
            ...
            data['mapperCode'] = contentItem['mapper_code']
            ...
            data['permissionMapperId'] = contentItem['permission_mapper_id']
            ...
            data['flagName'] = top.cardone.configuration.utils.DictionaryUtils.readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'permissionMapperFlag,flag', 'dictionaryCode': contentItem['flag_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['stateName'] = top.cardone.configuration.utils.DictionaryUtils.readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'permissionMapperState,state', 'dictionaryCode': contentItem['state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['dataStateName'] = top.cardone.configuration.utils.DictionaryUtils.readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'permissionMapperDataState,dataState', 'dictionaryCode': contentItem['data_state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])
