...
class r0002 implements java.io.Serializable {
    ...
    def func(input) {
		ApplicationContextHolder.getBean(UserGroupRoleService.class).findListByKeyword(input)
    }
    ...
    def output(output) {
        ...
        for (def outputItem : output) {
			newOutput.add(['label': outputItem['name'], 'value': outputItem['C1_USER_GROUP_ROLE_code']])
        }
