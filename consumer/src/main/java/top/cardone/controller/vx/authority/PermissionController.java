...
public class PermissionController {
    ...
    public Object d0001Json(HttpServletRequest request) {
        return ApplicationContextHolder.getBean(WebSupport.class).func(request,
                (Func1<Object, Map<String, Object>>) input -> ApplicationContextHolder.getBean(PermissionService.class).updateListCache((List) MapUtils.getObject(input, "permissionIds")));
    }
    /** /d0001.json end **/
    ...
    public Object d0002Json(HttpServletRequest request) {
        return ApplicationContextHolder.getBean(WebSupport.class).func(request,
                (Func1<Object, Map<String, Object>>) input -> ApplicationContextHolder.getBean(PermissionService.class).deleteListCache((List) MapUtils.getObject(input, "permissionIds")));
    }
    /** /d0002.json end **/
    ...
    public Object u0001Json(HttpServletRequest request) {
        return ApplicationContextHolder.getBean(WebSupport.class).func(request,
                (Func1<Object, Map<String, Object>>) input -> {
                    ...
                    readOne.put("permissionCode", MapUtils.getString(input, "permissionCode"));
                    readOne.put("object_id", "permissionId");

                    String dbPermissionId = ApplicationContextHolder.getBean(PermissionService.class).readOne(String.class, readOne);

                    if (dbPermissionId != null) {
                        String permissionId = MapUtils.getString(input, "permissionId");

                        if (!StringUtils.equals(dbPermissionId, permissionId)) {
                            throw new CodeException("permissionCode already exists", "许可编号已经存在");
                        }
                    }

                    return ApplicationContextHolder.getBean(PermissionService.class).updateCache(input);
                });
    }
    /** /u0001.json end **/
