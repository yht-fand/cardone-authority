...
class r0004 implements java.io.Serializable {
    ...
    def func(input) {
		ApplicationContextHolder.getBean(RolePermissionService.class).page(input)
    }
    ...
    def output(output) {
        ...
        for (def contentItem : output.getContent()) {
            ...
            data['permissionCode'] = contentItem['permission_code']

            data['permissionId'] = contentItem['permission_id']

            data['personalCode'] = contentItem['personal_code']

            data['personalId'] = contentItem['personal_id']

            data['roleCode'] = contentItem['role_code']

            data['roleId'] = contentItem['role_id']

            data['rolePermissionId'] = contentItem['role_permission_id']
            ...
            data['flagName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'rolePermissionFlag,flag', 'dictionaryCode': contentItem['flag_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['stateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'rolePermissionState,state', 'dictionaryCode': contentItem['state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])

            data['dataStateName'] = ApplicationContextHolder.getBean(DictionaryService.class).readOneByDictionaryTypeCodesCache(['dictionaryTypeCodes': 'rolePermissionDataState,dataState', 'dictionaryCode': contentItem['data_state_code'], 'stateCode': '1', 'dataStateCode': '1',  'object_id': 'name'])
