...
REPLACE((CASE WHEN (r.PARENT_TREE_CODE IS NULL OR LENGTH(r.PARENT_TREE_CODE) < 1) THEN r.ROLE_CODE ELSE CONCAT(r.PARENT_TREE_CODE, ',', r.ROLE_CODE) END),',', '/') AS ROLE_TREE_CODE,
t.`ROLE_CODE`,
r.`NAME` AS ROLE_NAME,
...
REPLACE((CASE WHEN (p.PARENT_TREE_CODE IS NULL OR LENGTH(p.PARENT_TREE_CODE) < 1) THEN t.PERMISSION_CODE ELSE CONCAT(p.PARENT_TREE_CODE, ',', t.PERMISSION_CODE) END),',', '/') AS PERMISSION_TREE_CODE,
t.`PERMISSION_CODE`,
