...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM C1_USER_GROUP_ROLE E
<#assign prefixName = true>
...
${prefixName?string('WHERE ', 'AND ')}E.USER_GROUP_ROLE_ID = :where_and_eq_userGroupRoleId_value
<#else>
${prefixName?string('WHERE ', 'AND ')}(E.USER_GROUP_ROLE_ID IS NULL OR E.USER_GROUP_ROLE_ID = '')
</#if>
<#assign prefixName = false>
</#if>
