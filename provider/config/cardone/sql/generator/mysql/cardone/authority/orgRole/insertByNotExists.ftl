...
<#if (insert_orgRoleId??) && (insert_orgRoleId_value??)>
${prefixName} `ORG_ROLE_ID`
<#assign prefixName = ','>
</#if>
...
<#if (insert_orgRoleId??) && (insert_orgRoleId_value??)>
${prefixName} :insert_orgRoleId_value
<#assign prefixName = ','>
</#if>
<#if (insert_permissionCodes??) && (insert_permissionCodes_value??)>
...
FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM c1_org_role E
...
<#if (where_and_eq_orgRoleId??)>
<#if (where_and_eq_orgRoleId_value??)>
${prefixName} E.ORG_ROLE_ID = :where_and_eq_orgRoleId_value
<#else>
${prefixName} E.ORG_ROLE_ID IS NULL
</#if>
<#assign prefixName = 'AND'>
</#if>
