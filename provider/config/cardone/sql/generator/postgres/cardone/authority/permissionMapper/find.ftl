...
<#if (order_by_mapperCode??)>
${prefixName?string('ORDER BY ', ', ')}"mapper_code" ${order_by_mapperCode_value!}
<#assign prefixName = false>
</#if>
<#if (order_by_orderBy??)>
...
<#if (order_by_permissionMapperId??)>
${prefixName?string('ORDER BY ', ', ')}"permission_mapper_id" ${order_by_permissionMapperId_value!}
<#assign prefixName = false>
</#if>
